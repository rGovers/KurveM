...
ScaleCurveNodeToolAction::ScaleCurveNodeToolAction(Workspace* a_workspace, Editor* a_editor)
{
    m_workspace = a_workspace;
    m_editor = a_editor;
}
ScaleCurveNodeToolAction::~ScaleCurveNodeToolAction()
{

}

bool ScaleCurveNodeToolAction::Interact(const glm::mat4& a_viewProj, const glm::vec3& a_pos, e_Axis a_axis, const Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize, CurveModel* a_model)
{
    ...
    if (SelectionControl::PointInMesh(mat, handle, a_cursorPos))
    {
        ...
        m_editor->PushAction(new ScaleCurveNodeAction(m_workspace, indices, nodeCount, a_model, cPos, axis, m_editor->GetMirrorMode()), "Error scaling curve node");
...
bool ScaleCurveNodeToolAction::LeftClicked(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
...
bool ScaleCurveNodeToolAction::LeftDown(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
    const glm::vec3 cWorldPos = a_camera->GetScreenToWorld(glm::vec3(a_cursorPos, 0.9f), a_screenSize);

    if (m_editor->GetCurrentActionType() == ActionType_ScaleCurveNode)
    {
        ScaleCurveNodeAction* action = (ScaleCurveNodeAction*)m_editor->GetCurrentAction();
...
bool ScaleCurveNodeToolAction::LeftReleased(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
    if (m_editor->GetCurrentActionType() == ActionType_ScaleCurveNode)
    {
...
void ScaleCurveNodeToolAction::Draw(Camera* a_camera)
{
    ...
    if (object != nullptr)
    {
        ...
        if (model != nullptr)
        {
            ...
            Gizmos::DrawScale(fPos, viewInv[2], ToolSettings::EditToolScale);
        }
    }    
}