...
RotateCurveNodeToolAction::RotateCurveNodeToolAction(Workspace* a_workspace, Editor* a_editor)
{
    m_workspace = a_workspace;
    m_editor = a_editor;
}
RotateCurveNodeToolAction::~RotateCurveNodeToolAction()
{

}

bool RotateCurveNodeToolAction::Interact(const glm::mat4& a_viewProj, const glm::vec3& a_pos, e_Axis a_axis, const Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize, CurveModel* a_model)
{
    ...
    if (SelectionControl::PointInMesh(mat, handle, a_cursorPos))
    {
        ...
        m_editor->PushAction(new RotateCurveNodeAction(m_workspace, indices, nodeCount, a_model, cPos, axis, m_editor->GetMirrorMode()), "Error rotating curve node");
...
bool RotateCurveNodeToolAction::LeftClicked(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
...
bool RotateCurveNodeToolAction::LeftDown(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
    const glm::vec3 cWorldPos = a_camera->GetScreenToWorld(glm::vec3(a_cursorPos, 0.9f), a_screenSize);

    if (m_editor->GetCurrentActionType() == ActionType_RotateCurveNode)
    {
        RotateCurveNodeAction* action = (RotateCurveNodeAction*)m_editor->GetCurrentAction();
...
bool RotateCurveNodeToolAction::LeftReleased(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
    if (m_editor->GetCurrentActionType() == ActionType_RotateCurveNode)
    {
...
void RotateCurveNodeToolAction::Draw(Camera* a_camera)
{
