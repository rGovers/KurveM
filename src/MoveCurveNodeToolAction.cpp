...
MoveCurveNodeToolAction::MoveCurveNodeToolAction(Workspace* a_workspace, Editor* a_editor)
{
    m_workspace = a_workspace;
    m_editor = a_editor;
}
MoveCurveNodeToolAction::~MoveCurveNodeToolAction()
{

}

bool MoveCurveNodeToolAction::Interact(const glm::mat4& a_viewProj, const glm::vec3& a_pos, e_Axis a_axis, const Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize, CurveModel* a_model)
{
...
bool MoveCurveNodeToolAction::LeftClicked(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
...
bool MoveCurveNodeToolAction::LeftDown(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
    const glm::vec3 cWorldPos = a_camera->GetScreenToWorld(glm::vec3(a_cursorPos, 0.9f), a_screenSize);

    if (m_editor->GetCurrentActionType() == ActionType_MoveCurveNode)
    {
        MoveCurveNodeAction* action = (MoveCurveNodeAction*)m_editor->GetCurrentAction();
...
bool MoveCurveNodeToolAction::LeftReleased(Camera* a_camera, const glm::vec2& a_cursorPos, const glm::vec2& a_screenSize)
{
    if (m_editor->GetCurrentActionType() == ActionType_MoveCurveNode)
    {
...
void MoveCurveNodeToolAction::Draw(Camera* a_camera)
{
